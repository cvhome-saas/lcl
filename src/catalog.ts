...
export function loadCatalog(configFile: string): Catalog {
    ...
    const { infra, composeServices, images } = readCompose(root, config.compose.file);
    void images;
    ...
    for (const name of Object.keys(config.services)) {
        ...
        const port = def.port;
        if (!Number.isInteger(port)) die(`lcl.yml: services.${name} needs a port`);
        ...
        if (def.type === 'container') {
            ...
            containers[name] = { compose, port: port!, containerPort: def.containerPort ?? port! };
            continue;
        }
        if (def.type === 'gradle' && !def.module) { unknown.push(`${name} (gradle without module)`); continue; }
        if ((def.type === 'npm' || def.type === 'exec') && !def.command) { unknown.push(`${name} (${def.type} without command)`); continue; }
        services.push({ name, port: port!, def });
    }
